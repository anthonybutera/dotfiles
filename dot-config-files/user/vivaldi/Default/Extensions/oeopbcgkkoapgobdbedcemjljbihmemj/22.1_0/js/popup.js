...
async function init() {
	...
	if (accounts.length >= 2 && await storage.get("drawer") != "closed") {
	...
	$body
		...
		.keydown(function(e) {
			...
			if (isFocusOnInputElement()) {
				...
				} else if ((keyboardException_R == "reply" && e.key == 'r') || e.key == 'a') {
	...
	polymerPromise.then(async () => {
		if (await shouldShowExtraFeature()) {
			...
			$("#newsNotification")
				...
				.click(async () => {
                    await storage.remove("_lastBigUpdate");
					openUrl("https://jasonsavard.com/wiki/Checker_Plus_for_Gmail_changelog?ref=bigUpdateFromPopupWindow");
				})
            ;
	...
	if (accounts.length == 0 || (accounts.length == 1 && accounts[0].error && accounts[0].error != "timeout" && accounts[0].getMailUrl().indexOf("/mail/") != -1)) {
        if (isOnline()) {
            let $dialog;
