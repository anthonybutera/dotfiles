...
async function initPopup() {
    ...
    chrome.browserAction.getPopup({}, result => {
        if (result && result.indexOf("setVia=manifest") != -1) {
...
function sendMessageToBG(command, params, stringifyParams = false) {
    ...
    if (window.inBackground) { // if running in same context
        if (command.indexOf(".") != -1) { // ie. forgottenReminder.start
...
async function retrieveAccounts(storageKey = "accounts") {
    ...
    const promises = accounts.map(async account => {
        ...
        if (account.mails) {
            try {
                account.mails = await Encryption.decrypt(account.mails);
        ...
        if (account.unsnoozedEmails) {
            try {
                account.unsnoozedEmails = await Encryption.decrypt(account.unsnoozedEmails);
